...
多層防御が**四層**になった。`tests/run.sh` は 103 項目すべて成功し、**push と PR のたびに
...
**表の PreToolUse 側の一マスが埋まった。** 人間が新しい対話セッションで
`rm -f /tmp/guardrails-manual-check` を頼むと確認ダイアログが出た（v2.1.220 では素通り
だった経路）。

**そのあと結論が二度ひっくり返った。** 作業セッション側で撃つと素通りに見えたので
「同じ環境でもセッションによって割れる」と結論し、記述を書き換えてコミットまでした。
だがそれは測り方の誤りで、単独コマンドで撃ち直したら確認は出た。**この一件が今日
いちばんの収穫**——経緯と教訓は「確認の不在は、エージェントには測れない」に残した。
上の表より価値がある。
...
- `selfcheck.py` の注意書き——環境で挙動が変わることと、単独コマンドで人間が見ること。
  毎セッション出る文なので長さは増やさず正確さだけ上げた
- `README.md`——版・機ごとの実測表を追加し、「`ask` 経路そのものが機能しない」の断定を撤回。
  **「確認が出なかったはエージェントには測れない」節を新設**。テスト件数の 85 も
  実測値へ直した（腐っていた）
