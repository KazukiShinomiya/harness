...
多層防御が**四層**になった。`tests/run.sh` は 88 項目すべて成功し、**push と PR のたびに
...
**`chattr +i` の判断材料が古くなっていたので直した。** `status` を実測したら三候補とも
「通常」。掛けるかを考えて三つとも見送りとしたが、その過程で `immutable.sh` の代償記述が
`setup.sh` 管理へ移した今日の変更を知らないままだったと分かった（README の表も同じ）。
`status` の末尾も、実体コピーか symlink かを**測って**出すよう変えた——`+i` の代償はそこで
分岐するのに、従来は「実体が〜のことがある」という一般論を印字していただけだった。
テストを 3 件足したが、**旧実装へ戻した複製で 2 件がそのまま通った**（詳細は決定事項）。
