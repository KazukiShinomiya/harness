...
**A では最初から一度も確認が出ていない**（人間が証言。この日の全ツール呼び出しを通じて
ダイアログを一度も見ていない）。途中で効かなくなったのではなく、始めから効いていない。
**何がセッション差を生むかは未特定**——A と B の間で違うのは、A が SESSION_STATE を
読み込んでから長く続いた作業セッションであることくらいで、設定側に差は見つからなかった。
...
1. **上流 [#79356](https://github.com/anthropics/claude-code/issues/79356) へ続報を出す。**
   ...
   **セッションを開いたら一度撃って記録する習慣にすると、この手の切り分けが早く済む。**
   今回は A で最初から効いていなかったことを人間の証言で埋めたが、実測が残っていれば早かった。
