...
**この機・この版では PreToolUse の `ask` は効く。** ネイティブ Linux 6.8 / Ubuntu 24.04.4
（**WSL2 ではない**）、v2.1.226。単独コマンドを複数回撃ち、人間が確認ダイアログを目視した。

| 版 / 機 | 経路 | 結果 |
|---|---|---|
| v2.1.220 / WSL2（Linux 5.15） | PreToolUse の ask | 素通し |
| **v2.1.226 / この機** | **PreToolUse の ask** | **確認ダイアログが出る** |
| v2.1.226 / この機 | `permissions` の ask | 素通し（**要再検証**。下記） |
...
**`permissions.ask` の素通しは、いま信用してはいけない。** 2026-08-09 の実測は下記の
測定ミスと同じ穴を持っている。単独コマンドで人間が目視して撃ち直すまで未確定として扱う。
...
**表の最後の一マスが埋まった。** 人間が新しい対話セッションで
`rm -f /tmp/guardrails-manual-check` を頼むと確認ダイアログが出た（v2.1.220 では素通り
だった経路）。この機・この版では PreToolUse の `ask` は効く。
...
1. **`permissions.ask` を単独コマンド＋目視で撃ち直す。** 上流へ出せるかはこれ次第。
   2026-08-09 の「素通し」は測定ミスと同じ穴を持っており、いま信用できない。
   `deny` を対照に置くのは有効だが、**`deny` が効いたことを `ask` 側の説明に使わない**
   （`deny` は `allow` に勝つ）。人間が画面を見て「出なかった」を確認すること。
2. **上流 [#79356](https://github.com/anthropics/claude-code/issues/79356) への続報は 1 の後。**
   現時点で言えるのは「v2.1.226 / ネイティブ Linux では PreToolUse 経由の `ask` は通る」
   だけで、これは修正報告にはならない（v2.1.220 の実測は WSL2 機なので版と機が交絡）。
   `permissions.ask` が本当に残っているなら、そちらが報告に値する。文面の素材は
...
4. `chattr +i` は未適用のまま（2026-08-09 に `status` を実測。三候補とも「通常」）。
...
5. `session-harness` は初版のまま手付かず。急ぐ理由は無い（動いてはいる）。
