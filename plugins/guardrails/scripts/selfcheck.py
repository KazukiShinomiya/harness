...
要件は実際の誤診からそのまま来ている:
  ...
  - PreToolUse フックの permissionDecision "ask" が Claude Code 側で表示されず、
    ...
    ネイティブ Linux 機の v2.1.226 / v2.1.229 では両経路とも表示される（いずれも
    単独コマンドで撃ち、人間が目視）。**「セッションによって割れる」と一度書いたが、
    それは複合コマンドで測った誤りだったので撤回した**（2026-08-10）。版とマシンが
    同時に違うため、素通しの原因が版なのかマシンなのかは切り分けられていない。
    → decision が ask のときは、環境で挙動が変わることと手動確認の手順を添える。
...
def emit(plugin_problems, layers, root):
    ...
    if current == "ask":
        caveat = (
            ...
            "（v2.1.220 の WSL2 機で実測。v2.1.226 / v2.1.229 のネイティブ Linux 機では"
