...
| v2.1.226 | ネイティブ（Linux 6.8） | `permissions` の `ask` | 確認が出ず素通し |

`deny` はどの版・どの機・どちらの経路でも効く。フックが発火していることも別途
確かめてある（`guard.sh` の先頭にタイムスタンプを追記させると、Bash 呼び出しごとに
ログが 2 行増える——`hooks.json` の PreToolUse 定義 2 件が `Bash` にマッチするため）。

**版と機が同時に違うので、PreToolUse 経由が通るようになった原因は切り分けられていない。**
同じ機で両方の版を撃つまでは「v2.1.226 で直った」と言い切らないこと。
...
- [x] **`ask` の不発は環境依存。現行版では PreToolUse 経由は通る。** v2.1.220（WSL2）では
      フック経由でも `permissions` の `ask` ルールでも確認が出なかったが、v2.1.226
      （ネイティブ Linux）では PreToolUse 経由の確認が出る。`permissions.ask` は素通しのまま。
      版と機が同時に違うので原因は未切り分け。`deny` はどの条件でも効くので、確実に
