...
| **v2.1.229** | ネイティブ（Linux 6.8） | **PreToolUse の `ask`** | **確認ダイアログが出る** |
| **v2.1.229** | ネイティブ（Linux 6.8） | **`permissions` の `ask`** | **確認ダイアログが出る** |
...
**確認の出どころは文言に現れる。** PreToolUse 由来は
`Hook PreToolUse:Bash requires confirmation for this command:` に続けて判定器の理由文が並び、
末尾に `[plugin:guardrails]` が付く。`permissions` 由来は
`Permission rule Bash(echo:*) requires confirmation for this command.` と**一致したルールを
名指しする**。ただし**ルールに当たらない既定の確認の文言は観測していない**ので、文言だけで
`ask` 由来と断定はできない。`allow` を上書きする測り方は依然として必要。

**版とマシンが同時に違うので、両経路が通るようになった原因は切り分けられていない。**
同じマシンで両方の版を撃つまでは「新しい版で直った」と言い切らないこと。
v2.1.226 と v2.1.229 はどちらもこのネイティブ Linux 機での実測で、素通しの記録は
すべて WSL2 機の v2.1.220 のもの。**版の違いなのかマシンの違いなのかは、まだ分かっていない。**
