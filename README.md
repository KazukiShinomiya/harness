...
`ask` が黙って表示されなくなり、v2.1.226 では同じ機でもセッションによって出たり出なかったりした）。
...
**確認が出るかはセッションに依る**（[詳細](#ask-は環境によって黙って消える実測)）。同じ機・
同じ版でも出るセッションと出ないセッションがあることを実測した。確認が出なくても、
...
`ask` という決定そのものだと結論した。**再検証でもっと悪いことが分かった——
`ask` が表示されるかどうかは、同じ環境でもセッションによって変わる。**
...
| v2.1.226 | ネイティブ（Linux 6.8） | **A** | PreToolUse の `ask` | **確認が出ず素通し** |
| v2.1.226 | ネイティブ（Linux 6.8） | **B** | PreToolUse の `ask` | **確認ダイアログが出る** |
| v2.1.226 | ネイティブ（Linux 6.8） | A | `permissions` の `ask` | 確認が出ず素通し |

**A と B は同一機・同一版・同一 cwd・同一 `permissionMode`（default）・同一設定。**
違うのはセッションだけで、同じ `rm -f /tmp/guardrails-manual-check` に対して
片方は素通しし、もう片方は確認を出した。

セッション A では素通しの原因も切り分けてある。**フックは発火しており、出力も
届いている**——`guard.sh` の先頭にタイムスタンプを追記させると Bash 呼び出しごとに
2 行増え（`hooks.json` の PreToolUse 定義 2 件が `Bash` にマッチする）、判定器の既定を
`deny` に差し替えると同じコマンドが即座にブロックされた。落ちているのは `ask` だけ。

`deny` はどの版・どの機・どのセッションでも効く。これは対照として毎回同時に
確かめている——`deny` が効くことは「設定・フックが読み込まれている」証拠でもあり、
素通りを「読まれていない」で説明できなくする。

**したがって「自分の環境で一度確認が出たから大丈夫」とは言えない。** 次のセッションで
同じ保証は無い。何がセッション差を生むかは未特定。

**測り方に注意。** 対話セッションでしか測れない。`claude -p`（非対話）では確認を出す
相手がいないため `ask` は `denied` に倒れ、「止まった」が「直った」に見える。
上の表の対話行は人間が目視して埋めたもので、`-p` で得ていた `denied` は
証拠として採用していない。
...
今そのセッションで `ask` が表示されるかは一発で確かめられる。**対話セッションで**
次を実行させる。

```bash
rm -f /tmp/guardrails-manual-check   # 確認が出なければ、そのセッションでは素通しになる
```

このファイルは存在しないので、確認に `Yes` と答えても副作用は無い。

**分かるのは「今このセッションで出るか」だけだ。** 上の表のとおり同じ環境でも
セッションによって変わるので、一度出たことを恒久的な保証として扱わないこと。
...
を dotfiles 側の正本から用意するため。この機では `setup.sh` が正本と `settings.machine.json` を
...
- [x] **`ask` の表示はセッションによって変わる。** v2.1.220（WSL2）ではフック経由でも
      `permissions` の `ask` ルールでも確認が出なかった。v2.1.226（ネイティブ Linux）では
      **同一機・同一版・同一 `permissionMode` でも、あるセッションは素通しし、別のセッションは
      確認を出す**。素通しした側でもフックは発火し `deny` は効いていた（`guard.sh` への
      タイムスタンプ追記と、判定器を `deny` に差し替えた対照で確認）。落ちるのは `ask` だけ。
      何がセッション差を生むかは未特定。`deny` はどの条件でも効くので、確実に止めたいものは
      `deny` か git 層（[詳細](#ask-は環境によって黙って消える実測)）
