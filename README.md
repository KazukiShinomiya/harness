...
`ask` が黙って表示されなくなり、別の機の v2.1.226 では表示された——環境で挙動が変わる）。
...
**確認が出るかは環境に依る**（[詳細](#ask-は環境によって黙って消える実測)）。実測した
2 環境のうち片方では 1・2 とも確認が出て、もう片方では出なかった。確認が出なくても、
...
`ask` という決定そのものだと結論した。**この結論は再検証で半分外れた。**

#### 実測（Linux / Bash）

| 版 | 機 | 経路 | 結果 |
|---|---|---|---|
| v2.1.220 | WSL2（Linux 5.15） | PreToolUse フックの `ask` | 確認が出ず素通し |
| v2.1.220 | WSL2（Linux 5.15） | `permissions` の `ask` ルール | 確認が出ず素通し |
| **v2.1.226** | ネイティブ（Linux 6.8） | **PreToolUse フックの `ask`** | **確認ダイアログが出る** |
| v2.1.226 | ネイティブ（Linux 6.8） | `permissions` の `ask` ルール | 確認が出ず素通し |

`deny` はどの版・どの機・どちらの経路でも確実に効く。これは対照として毎回同時に
確かめている——`deny` が効くことは「設定・フックが読み込まれている」証拠でもあり、
素通りを「読まれていない」で説明できなくする。

**確定していること: 経路によって挙動が違う。** 下 2 行は同一機・同一版で取っているので
交絡が無い。`permissions.ask` だけが素通しになる。

**確定していないこと: 何が直したのか。** 上 2 行と下 2 行は版も機も違う。PreToolUse の
`ask` が出るようになった理由が版の修正なのか機の差なのかは、この 4 行だけでは切り分け
られない。同じ機で両方の版を撃つまでは「v2.1.226 で直った」と言い切らないこと。

いずれにせよ**プラグイン側の `ask` に寄りかからない**——配布先の版も機も選べない。
確実に止めたいものは引き続き `deny` と git 層へ寄せる。

**測り方に注意。** 対話セッションでしか測れない。`claude -p`（非対話）では確認を出す
相手がいないため `ask` は `denied` に倒れ、「止まった」が「直った」に見える。
v2.1.226 の PreToolUse 行は人間が対話セッションで目視して埋めたもので、
それ以前に `-p` で得ていた `denied` は証拠として採用していない。
...
自分の環境で `ask` が表示されるかは一発で確かめられる。**対話セッションで**次を実行させる。
...
- [x] **`ask` の不発は `permissions` ルート限定だった。** v2.1.220（WSL2）ではフック経由でも
      `permissions` の `ask` ルールでも確認が出なかったが、v2.1.226（ネイティブ Linux）では
      **同一条件でも経路によって割れる**——PreToolUse 経由は確認が出て（対話セッションで目視）、
      `permissions.ask` は素通しのまま。何がその差を生んだか（版か機か）は未切り分け。
      `deny` はどの環境・どちらの経路でも効く。環境に依存しない強制力が要るなら `deny` か
      git 層（[詳細](#ask-は環境によって黙って消える実測)）
