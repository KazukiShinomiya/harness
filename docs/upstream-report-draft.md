...
**Reproduction 1 も 2 も、v2.1.226（ネイティブ Linux 6.8、Ubuntu 24.04.4）では再現しない。**
PreToolUse フックが返した `ask` も、`permissions.ask` ルールも、確認ダイアログとして
表示される。どちらも単独コマンドで撃ち、人間が画面で確認した。

| 経路 | v2.1.220 / WSL2 | v2.1.226 / ネイティブ | v2.1.229 / ネイティブ |
|---|---|---|---|
| Reproduction 1（PreToolUse の `ask`） | 素通し | **確認が出る** | **確認が出る** |
| Reproduction 2（`permissions.ask`） | 素通し | **確認が出る**（08-10 に測り直し） | **確認が出る**（08-13） |
...
**この二点は Reproduction 2 にも当てはまっていた。** `permissions.ask` の素通しも
同じ穴のある測り方で得た記録だったので、2026-08-10 に撃ち直した——`allow` にある
`Bash(echo:*)` を `ask` で上書きし、`echo` を単独で撃つ。確認は出た。
**この機・この版では両経路とも壊れていない。**

そのうえで、**上流へ出せる新事実は無い**というのが現時点の判断。投稿済みコメントは
v2.1.220 / WSL2 の実測で、今回の結果はそれを否定しない（版も機も違う）。
「v2.1.226 で直った」と書くには同じ機で両方の版を撃つ必要があり、それはまだできていない。
